...
cont.ranges<-weight_avg(bwidth = 0.1)
...
save.image("conti2rsim1_010_090g_lamda_01.RData")
...
################### Now plotting the deformed space for a randomly chosen simulation run ############
library(viridis)

N=70
set.seed(100)
check.index<-sample(1:100,1)
error<-sim_summary[[check.index]]$data
grid<-sim_summary[[check.index]]$grid
deform_ordered<-sim_summary[[check.index]]$def.coord
quilt.plot(grid[,-3],c(error),nx=N,ny=N)
mle.def<-sim_summary[[check.index]]$mle.def
psi<-sim_summary[[check.index]]$psi
##### first re_ordering deformed coordinates ######
par(mfrow=c(1,1))
par(mar=c(4,4,0.5,1))
quilt.plot(grid[,-3],c(error),nx=N,ny=N,col=viridis(n=5000),horizontal=T,xlab="x",ylab="y")
abline(v=1)
x<-seq(0,2,length=N)
y<-seq(0,2,length=N)
...
pp=pi/2
deform_ordered3<-deform_ordered[,1:3]
...
for(j in 1:N)
{
  ...
  s3$points3d(x=xvals,y=yvals,z=zvals,type = "l",cex=0.1,col="grey")
  
}
s3$points3d(x=deform_ordered3[,1],y=deform_ordered3[,2],z=deform_ordered3[,3],pch=15,cex = 0.4,col=color.scale(error,col=viridis(n=5000),zlim=c(min(error)-0.001,max(error)+0.001)))

image.plot(legend.only = T,zlim=c(min(error)-0.001,max(error)+0.001),horizontal = T,legend.width = 0.5,col = viridis(n=5000),legend.mar = 2 )
...
quilt.plot(grid[,-3],my_Cns[1910,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 1")
quilt.plot(grid[,-3],my_Cns[1792,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 2")

quilt.plot(grid[,-3],my_Cns[1800,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 2")


quilt.plot(grid[,-3],def_cov[1910,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 1")
quilt.plot(grid[,-3],def_cov[1792,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 2")

quilt.plot(grid[,-3],def_cov[1800,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 3")



quilt.plot(grid[,-3],my_Cns[1910,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 1",col = viridis(n=5000))
quilt.plot(grid[,-3],my_Cns[1792,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 2",col = viridis(n=5000))

quilt.plot(grid[,-3],my_Cns[1800,]/my_Cns[1,1],nx=N,ny=N,main="True correlation at location 2",col = viridis(n=5000))


quilt.plot(grid[,-3],def_cov[1910,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 1",col = viridis(n=5000))
quilt.plot(grid[,-3],def_cov[1792,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 2",col = viridis(n=5000))

quilt.plot(grid[,-3],def_cov[1800,]/def_cov[1,1],nx=N,ny=N,main="Estim corr at location 3",col = viridis(n=5000))



######## Plotting correlation maps using ggplot #######
library(ggplot2)
...
sl.locs<-c(1910,1792,1800)
temp_mycat<-temp_truecat<-numeric()
temp_locs<-NULL
for(i in 1:3)
{
  temp_mycat<-c(temp_mycat,def_cov[sl.locs[i],]/def_cov[1,1])
  temp_truecat<-c(temp_truecat,my_Cns[sl.locs[i],]/my_Cns[1,1])
  
  temp_locs<-c(temp_locs,paste("Location ",i))
}


data_heat<-data.frame(x=rep(grid$x,times=6),
                      y=rep(grid$y,times=6),
                      z=c(temp_mycat,temp_truecat),
                      locs=c(rep(temp_locs,each=length(grid$x)),
                             rep(temp_locs,each=length(grid$x))),
                      Model=rep(c("Estimated correlation","True correlation"),each=3*length(grid$x)))
...
##################################################################################################
############################ Now we plot the alignment results ###################################
##################################################################################################


my_r1.v<-r1.v[[check.index]]
my_r2.v<-r2.v[[check.index]]
#plot(time,my_r1.v)
#points(time,my_r2.v)

####### Standardizing estimated variograms #######
my_r1.v<-my_r1.v/max(my_r1.v)
my_r2.v<-my_r2.v/max(my_r2.v)
...
par(mar=c(4,4,0.2,6))
plot(NA,NA,xlim = c(0,2),ylim=c(0,2),ylab = "y",xlab = "x")
abline(v=x,col="grey")
abline(h=y,col="grey")
quilt.plot(grid$x,grid$y,error,col = viridis(n=5000),add=T,horizontal = F,legend.width = 0.5)
abline(v=1)
par(mar=c(4,4,0.5,0.5))
...
std.est.var<-data.frame(h=c(time,time),
                        v=c(my_r1.v,
                            my_r2.v),
                        Subregion=c(rep("Subregion 1 (G_1)",times=length(time)),rep("Subregion 2 (G_2)",times=length(time))))
ggplot(std.est.var, aes(x=h, y=v, color=Subregion)) + 
  geom_line(aes(x=h,y=v,col=Subregion,lty=Subregion))+
  labs(x=expression(paste("||",bold(h),"||")),y=expression(paste(hat(gamma)[i],"(||",bold(h),"||)")))+
  theme(legend.position="bottom")+
  scale_linetype_discrete(labels=c(bquote("Subregion 1 ("~G[1]~")"),bquote("Subregion 2 ("~G[2]~")")))+scale_color_discrete(labels=c(bquote("Subregion 1 ("~G[1]~")"),bquote("Subregion 2 ("~G[2]~")")))
####### Standardised aligned variogram ########
align.var<-data.frame(h=c(time,time),
                      v=c(my_out[[check.index]]$fn[,1]/max(my_out[[check.index]]$fn[,1]),
                          my_out[[check.index]]$fn[,2]/max(my_out[[check.index]]$fn[,2])),
                      Subregion=c(rep("Subregion 1 (G_1)",times=length(time)),rep("Subregion 2 (G_2)",times=length(time))))
...
regdist.war<-data.frame(h=c(time,time),
                        v=c(invertGamma(my_out[[check.index]]$gam[,1])*max(time),
                            invertGamma(my_out[[check.index]]$gam[,2])*max(time)),
                        Subregion=c(rep("Subregion 1 (G_1)",times=length(time)),rep("Subregion 2 (G_2)",times=length(time))))
